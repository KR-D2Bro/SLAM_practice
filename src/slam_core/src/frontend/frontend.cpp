...
int8_t Frontend::run(cv::Mat &img){
    ...
    if(keyframes.size()==0){
        ...
        //새로운 키프레임 추가
        feature_tracker_->track_feature(*last_keyframe, *cur_frame);
        if(visual_odometry_->pose_estimate_2d2d(*last_keyframe, *cur_frame, feature_tracker_->matches)){
        ...
        if(feature_tracker_->match_3d_2d(*map_points_, *cur_frame, points_3d, points_2d)){
            //Pnp 수행.
            if(visual_odometry_->PnPcompute_g2o(points_3d, points_2d, *cur_frame)){
                ...
                const bool c2 = num_inliers < last_keyframe->observed_map_points_.size() * 0.5 && num_inliers > 15;
                ...
                const double min_rot = 5.0 * M_PI / 180.0;
                ...
                c_parrallax = visual_odometry_->check_parrallax(*last_keyframe, *cur_frame, feature_tracker_->matches, 2.0);
